...
**Status:** Phase 1 (GROUND) complete — claim cards written (11-01…11-05). Phase 2 (CHANGE) executed — 7 comment-only hold declarations + stage-11 probe (8 archetypes × 2 formats) + claim 11-05 probe-side bundle fix (webpackOverride → subpackage 4.0.507 tree). Phase 3 (COUNTER-CHECK) — probe report ALL GREEN (C17 16/16 identical, hold-static 16/16, D6 2 sites w/ exception, bundle-single-copy PASS); dispatch 1 → CONFIRM (no rework needed); dispatch 2 (claim 11-05, bundle fix) → run exit 0, zero pass:false — CONFIRM.
...
| Item | Value |
|---|---|
| Repo HEAD | `29c3038` at snapshot time; now `0784e0b` — "feat(video-review): frame extraction + pixel audit render gate" (intervening commits: publish-queue work, 4d0f6cd, 986020d, a901893, bd59242 "wip: local changes from opencode session" which tracked `data/audit/11/` incl. out/ PNGs) |
| Working tree | Claim 11-05 in flight: `audit-motion.ledger.md` + `motion-probe.mjs` modified (my Phase 1/2 work). `out/` PNGs modified by pre-fix diagnosis runs (crashed-run artifacts — regenerated by the fixed probe). Untracked: `ci-artifacts-*`/`scratch-*`/`_gate-run.txt` (not mine), `data/audit/10/`, `src/skills/remotion-render/captions/` |
...
Bundle-skew evidence (claim 11-05, this phase): root `@remotion/bundler` 4.0.505 `dist/webpack-config.js:96,133-134` + `dist/shared-bundler-config.js` (getResolveConfig aliases `remotion` → root esm); served-bundle forensics (`var VERSION = "4.0.505"`; module comment `;// ./src/skills/remotion-render/node_modules/@remotion/media/...`; `useRenderMediaCache` → `Internals.Xp.RenderResourceManagerContext`; `// src/caches.ts`); subpackage remotion 4.0.507 `dist/esm/index.mjs:11770/12902` (RenderResourceManagerContext defined/exported — absent from root 4.0.505); beats audio mounts HeroNumber.jsx:3,112-114 / ListItem.jsx:3,217-219 / Progress.jsx:80,362-364; `src/skills/remotion-render/render.js` (subpackage @remotion/bundler — production single-copy/consistent).
...
### CLAIM-MOT-11-05 — Probe bundle single-copy fix: remotion + @remotion/* resolve to the subpackage 4.0.507 tree (audio frames render green)

```
ASSERTION   The stage-11 probe's webpack bundle currently mixes remotion
            versions: the bundler's getResolveConfig() aliases `remotion`
            (and remotion/no-react, remotion/version) to require.resolve
            ('remotion') from the ROOT node_modules (4.0.505 esm) for every
            module in the bundle, while the beats' @remotion/media resolves
            to the SUBPACKAGE 4.0.507 (src/skills/remotion-render/node_modules)
            whose dist inlines AudioForRendering → useRenderMediaCache →
            react.useContext(esm/* Internals */.Xp.RenderResourceManagerContext)
            (plus // src/caches.ts). Root 4.0.505's Internals does NOT export
            RenderResourceManagerContext (4.0.507-only: subpackage
            index.mjs:11770 defined, :12902 exported) → useContext(undefined)
            → "Cannot read properties of undefined (reading '_currentValue')"
            on every frame that mounts <Audio> (hero@66, list@18/28/41/45,
            progress@38 — HeroNumber.jsx:3,112-114; ListItem.jsx:3,217-219;
            Progress.jsx:80,362-364). runGate() (motion-probe.mjs:777-799)
            catches the render error SILENTLY (pushes {pass:false, note:
            "render error: …"} at :791-795, no console line), then the C17
            loop (:827+, no try/catch) crashes the probe before
            motion-report.json is written → exit 1, no evidence.
            After this change the probe's bundle() call gains webpackOverride
            re-aliasing remotion, remotion/no-react, remotion/version AND
            @remotion/media, @remotion/paths, @remotion/shapes to the
            subpackage tree's absolute esm paths (all 4.0.507; module fields
            verified dist/esm/index.mjs) → the bundle holds ONE remotion
            4.0.507 copy whose Internals exports RenderResourceManagerContext
            → every audio-mounting frame renders; runGate's catch prints the
            render error (no more silent failures); a post-bundle single-copy
            assertion (the probe reads the served bundle.js itself — external
            tool reads of the Temp bundle are permission-denied) fails the run
            if `"4.0.505"` is present (the root VERSION constant — its only
            occurrence in the bundle), `"4.0.507"` absent, or
            RenderResourceManagerContext absent. The full REV-3 schedule then
            runs green both formats — completion = motion-report.json with
            pass:true and zero pass:false rows (50 gate rows + 16 C17 rows +
            D6 + bundle). No gate logic changed.
SPEC REF    CROSSCHECK-PROTOCOL.md Part 2 (P1.1 claim card before edits;
            P2.1 delete-then-replace; P2.2 one claim one change).
            Orchestrator scope (11-05 diagnosis accepted): probe-side
            single-copy fix inside data/audit/11 only — do NOT edit
            package.json/node_modules (orchestrator-owned); if the probe-side
            fix proves impossible → SFR-MOT-11-8 and stop.
            @remotion/bundler (root 4.0.505) dist: webpack-config.js:96
            (resolve: getResolveConfig()) and :133-134 (webpackOverride
            applied AFTER the alias config — the override wins);
            shared-bundler-config.js getResolveConfig alias keys (remotion,
            remotion/no-react, remotion/version, react, react/jsx-runtime,
            react-dom/client, …); bundle.js:112,367 (options.webpackOverride
            forwarded into webpackConfig); bundle.d.ts (WebpackOverrideFn =
            (currentConfiguration) => Configuration | Promise<Configuration>).
SOURCES     [1] Live @remotion/bundler root 4.0.505: webpack-config.js
            :49,96,133-134; shared-bundler-config.js; bundle.js:112,367;
            bundle.d.ts (MandatoryBundleInternalsOptions.webpackOverride).
            [2] Served-bundle forensics (Temp remotion-webpack-bundle-zV8B2g/
            bundle.js + cache-purged re-run — byte-identical SHA256
            86A0208BA6BC): `var VERSION = "4.0.505"`; module comment `;// ./
            src/skills/remotion-render/node_modules/@remotion/media/dist/esm/
            index.mjs` (3 subpackage @remotion modules; mediabunny also
            subpackage); useRenderMediaCache module: react.useContext(esm/*
            Internals */.Xp.RenderResourceManagerContext); `// src/caches.ts`
            inlined; RenderResourceManagerContext occurrences = 1 (media
            reference only — the 4.0.505 definition absent).
            [3] Subpackage src/skills/remotion-render/node_modules: remotion
            4.0.507 (dist/esm/index.mjs RenderResourceManagerContext :11770
            defined / :12902 exported; dist/esm/no-react.mjs + version.mjs
            present — `var VERSION = "4.0.507"`); @remotion/media|paths|
            shapes 4.0.507 (module = dist/esm/index.mjs); react 19.2.8 ==
            root react 19.2.8 (no react skew).
            [4] Beats audio mounts: HeroNumber.jsx:3,112-114; ListItem.jsx:3,
            217-219; Progress.jsx:80,362-364.
            [5] motion-probe.mjs (live): bundle() :766-770 (no webpackOverride
            today); runGate :777-799 (silent catch :791-795); C17 loop
            :827-850 (crash site); report write :858-859; readFileSync
            (:49) + resolve (:50) already imported — no new imports needed.
            [6] Production impact: src/skills/remotion-render/render.js lives
            IN the skill dir → its @remotion/bundler import resolves to the
            SUBPACKAGE (4.0.507 present) → the production render pipeline is
            single-copy/consistent; the 4.0.505/4.0.507 skew is PROBE-ONLY
            (the probe imports @remotion/bundler from root).
RE-VERIFIED YES: every alias key, module entry field, version string and the
            context symbol's presence/absence re-checked from live node_modules
            this phase; webpackOverride plumbing verified end-to-end (bundle()
            → webpackConfig → applied after getResolveConfig).
CURRENT     bundle() at motion-probe.mjs:766-770 passes no webpackOverride;
            audio-mounting gate frames fail silently inside runGate, the C17
            loop crashes the probe → no report → exit 1. The crash reproduces
            on a cache-purged re-run with a byte-identical bundle (cache is
            not the cause; the earlier ALL-GREEN report predates the current
            dependency tree).
DELTA       One file, one logical change — data/audit/11/motion-probe.mjs:
            (a) bundle({...}) gains webpackOverride re-aliasing remotion,
            remotion/no-react, remotion/version, @remotion/media,
            @remotion/paths, @remotion/shapes → subpackage 4.0.507 absolute
            esm paths; (b) post-bundle single-copy assertion (probe reads
            serveUrl/bundle.js; report.bundle + failures++ on fail);
            (c) runGate catch (:791-795) console.logs the render error (was
            silent — the orchestrator's full gate-by-gate evidence
            requirement). No gate logic changed; C17/D6/G1-G6 untouched;
            no other files touched.
PLAN        Delete: the bundle({entryPoint, publicDir, onProgress}) call
            (:766-770). Replace: same call + webpackOverride (deleted block
            re-inserted with the override — P2.1); insert the bundle
            assertion between serveUrl (:771) and openBrowser (:773), wired
            into report after :775; insert the FAIL log in runGate catch.
DIFF        motion-probe.mjs only (working tree currently identical to HEAD —
            the whole fix is one delete-then-replace of the bundle() call
            plus two small insertions). Counter-check: verify-independent
            (claim + diff + files + gates only).
COUNTER     CONFIRMED (run, 2026-08-12): probe exit 0, "ALL GATES GREEN".
            motion-report.json: pass=true, bundle-single-copy PASS ("single
            remotion copy (subpackage 4.0.507) + RenderResourceManagerContext
            present"), gates 50 rows + c17 16 rows + d6 — ZERO pass:false
            anywhere (gates 0/50, c17 0/16). The frames that crashed before
            the fix now render and pass: hero@66 (G1 + hold-static + G2a),
            list@18/28/41/45 (G6a/G6c/hold-static/G6b), progress@38
            (hold-static), image@30/66 (image-push/G1/hold-static) — both
            formats. C17 16/16 pixel-identical (shorts + longform, declared
            pairs per §0.3), hold-static 16/16, D6 exactly 2 spring sites
            (ImageBeat ζ=10.000 A3.3 exception, Progress ζ=0.518).
            Dispatch 2 (bundle fix) → CONFIRM.
STATUS      IMPLEMENTED
```
