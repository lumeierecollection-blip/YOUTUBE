...
const staged = mg.beats.filter((b) => b.archetype !== "LIST_ITEM");
const target = wantStrategy
  ? staged.find((b) => b.visualPlan && b.visualPlan.strategy === wantStrategy)
  : staged[0];
if (!target) {
  console.error(
    `No beat planned ${wantStrategy}. This video used: ` +
      [...new Set(staged.map((b) => b.visualPlan && b.visualPlan.strategy))].join(", ")
  );
  process.exit(1);
}

// A little lead-in/out so the clip doesn't start hard on the first state.
const from = Math.max(target.startFrame - 6, 0);
const to = Math.min(target.startFrame + target.durationInFrames + 6, mg.totalFrames - 1);

console.log(`clip: ${target.visualPlan.strategy}  frames ${from}-${to}  (${((to - from) / 30).toFixed(1)}s)`);
console.log(`states: ${target.visualStates.map((s) => s.key).join(" > ")}`);
